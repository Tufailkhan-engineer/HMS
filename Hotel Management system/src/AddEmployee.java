package hotel.management.system;

import javax.swing.*;
import java.awt.*;
import java.awt.event.*;

public class AddEmployee extends JFrame implements ActionListener {

    JTextField tfname, tfemail, tfphone, tfage, tfsalary, tfadhaar;
    JRadioButton rbmale, rbfemale;
    JButton submit;
    JComboBox cbjob;

    AddEmployee() {
        setLayout(null);

        // creating name content on the frame //
        JLabel lblname = new JLabel("Name");
        lblname.setBounds(60, 30, 120, 30);
        lblname.setFont(new Font("Tahoma", Font.PLAIN, 17));
        add(lblname);

        // text field inform of the label //
        tfname = new JTextField();
        tfname.setBounds(200, 30, 150, 30);
        add(tfname);

        //--------------------------------------------------------------------------//
        // creating age content on the frame //
        JLabel lblage = new JLabel("Age");
        lblage.setBounds(60, 80, 120, 30);
        lblage.setFont(new Font("Tahoma", Font.PLAIN, 17));
        add(lblage);

        // text field inform of the label //
        tfage = new JTextField();
        tfage.setBounds(200, 80, 150, 30);
        add(tfage);

        //--------------------------------------------------------------------------//
        // creating content on the gender frame //
        JLabel lblgender = new JLabel("Gender");
        lblgender.setBounds(60, 130, 120, 30);
        lblgender.setFont(new Font("Tahoma", Font.PLAIN, 17));
        add(lblgender);

        rbmale = new JRadioButton("Male ");
        rbmale.setBounds(200, 130, 70, 30);
        rbmale.setFont(new Font("Tahoma", Font.PLAIN, 13));
        rbmale.setBackground(Color.WHITE);
        add(rbmale);

        rbfemale = new JRadioButton("Female ");
        rbfemale.setBounds(280, 130, 70, 30);
        rbfemale.setFont(new Font("Tahoma", Font.PLAIN, 13));
        rbfemale.setBackground(Color.WHITE);
        add(rbfemale);

        ButtonGroup bg = new ButtonGroup();
        bg.add(rbmale);
        bg.add(rbfemale);

        //--------------------------------------------------------------------------//
        // creating job content on the frame //
        JLabel lbljob = new JLabel("Job");
        lbljob.setBounds(60, 180, 120, 30);
        lbljob.setFont(new Font("Tahoma", Font.PLAIN, 17));
        add(lbljob);

        // dropdown //
        String str[] = {"Front Desk Clerk", "Porter", "Housekeeping", "Kitchen Staff", "Room Service", "Manager", "Concierge", "Maintenance", "Security", "Sales and Marketing", "Event Coordinator", "Accountant"};

        cbjob = new JComboBox(str);
        cbjob.setBounds(200, 180, 150, 30);
        cbjob.setBackground(Color.WHITE);
        add(cbjob);

        //--------------------------------------------------------------------------//
        // creating salary content on the frame //
        JLabel lblsalary = new JLabel("Salary");
        lblsalary.setBounds(60, 230, 120, 30);
        lblsalary.setFont(new Font("Tahoma", Font.PLAIN, 17));
        add(lblsalary);

        tfsalary = new JTextField();
        tfsalary.setBounds(200, 230, 150, 30);
        add(tfsalary);

        //--------------------------------------------------------------------------//
        // creating phone content on the frame //
        JLabel lblphone = new JLabel("Phone");
        lblphone.setBounds(60, 280, 120, 30);
        lblphone.setFont(new Font("Tahoma", Font.PLAIN, 17));
        add(lblphone);

        // text field inform of the label //
        tfphone = new JTextField();
        tfphone.setBounds(200, 280, 150, 30);
        add(tfphone);

        //--------------------------------------------------------------------------//
        // creating email content on the frame //
        JLabel lblemail = new JLabel("Email-ID");
        lblemail.setBounds(60, 330, 120, 30);
        lblemail.setFont(new Font("Tahoma", Font.PLAIN, 17));
        add(lblemail);

        // text field inform of the label //
        tfemail = new JTextField();
        tfemail.setBounds(200, 330, 150, 30);
        add(tfemail);

        //--------------------------------------------------------------------------//
        // creating Aadhaar content on the frame //
        JLabel lbladhaar = new JLabel("Aadhaar");
        lbladhaar.setBounds(60, 380, 120, 30);
        lbladhaar.setFont(new Font("Tahoma", Font.PLAIN, 17));
        add(lbladhaar);

        // text field inform of the label //
        tfadhaar = new JTextField();
        tfadhaar.setBounds(200, 380, 150, 30);
        add(tfadhaar);

        // creating button //
        submit = new JButton("Submit");
        submit.setForeground(Color.WHITE);
        submit.setBackground(Color.BLACK);
        submit.setBounds(200, 430, 150, 30);
        submit.addActionListener(this);
        add(submit);

        // adding image icon next to it //
        ImageIcon i1 = new ImageIcon(ClassLoader.getSystemResource("icons/tenth.jpg"));
        Image i2 = i1.getImage().getScaledInstance(450, 450, Image.SCALE_DEFAULT);
        ImageIcon i3 = new ImageIcon(i2);
        JLabel image = new JLabel(i3);
        image.setBounds(380, 60, 450, 370);
        add(image);

        // creating frame to be displayed //
        getContentPane().setBackground(Color.WHITE);
        setBounds(350, 200, 850, 540);
        setVisible(true);
    }

    // action listener //
    public void actionPerformed(ActionEvent ae) {
        String name = tfname.getText();
        String age = tfage.getText();
        String salary = tfsalary.getText();
        String phone = tfphone.getText();
        String email = tfemail.getText(); // Corrected from tfphone to tfemail
        String adhaar = tfadhaar.getText();

        String gender = null;
        if (rbmale.isSelected()) {
            gender = "Male";
        } else if (rbfemale.isSelected()) {
            gender = "Female";
        }

        String job = (String) cbjob.getSelectedItem();

        // Validation for phone and Aadhaar
        if (!phone.matches("\\d{10}")) {
            if (phone.length() != 10) {
                JOptionPane.showMessageDialog(null, "Phone number must be exactly 10 digits.");
            } else {
                JOptionPane.showMessageDialog(null, "Input should be a number.");
            }
            return;
        }

        if (!adhaar.matches("\\d{12}")) {
            if (adhaar.length() != 12) {
                JOptionPane.showMessageDialog(null, "Aadhaar number must be exactly 12 digits.");
            } else {
                JOptionPane.showMessageDialog(null, "Input should be a number.");
            }
            return;
        }

        try {
            // creating conn //
            conn c = new conn();
            // creating query //
            String query = "insert into employee values ('" + name + "', '" + age + "', '" + gender + "', '" + job + "', '" + salary + "', '" + phone + "', '" + email + "', '" + adhaar + "')";

            // executing query //
            c.s.executeUpdate(query);

            JOptionPane.showMessageDialog(null, "Employee Added successfully");

            setVisible(false);

        } catch (Exception e) {
            e.printStackTrace();
        }
    }

    public static void main(String[] args) {
        new AddEmployee();
    }
}
